...
def smoothed_signals(signal, win_len, pl_order=3, dlt=1., neuron_id=500, do_plots=False):
    smooth_cal = sig.savgol_filter(signal, window_length=win_len, deriv=0, delta=dlt, polyorder=pl_order)
    smooth_deriv = sig.savgol_filter(signal, window_length=win_len, deriv=1, delta=dlt, polyorder=pl_order)
