...
current_time = dtm.now()
time_str = current_time.strftime("%H:%M:%S")
with open('make-log.txt', 'a') as f:
    f.write('spikes splited, intra-cellular noise started to get simulated: ' + time_str + '\n')
...
time_str = current_time.strftime("%H:%M:%S")
with open('make-log.txt', 'a') as f:
    f.write('intra noise simulated, started to adding to spikes: ' + time_str + '\n')
...
time_str = current_time.strftime("%H:%M:%S")
with open('make-log.txt', 'a') as f:
    f.write('noisy spikes saved, recording noise started to get simulated: ' + time_str + '\n')
...
time_str = current_time.strftime("%H:%M:%S")
with open('make-log.txt', 'a') as f:
    f.write('recording noise finished, started to simulate calcium: ' + time_str + '\n')

spikes_noisy_1 = np.load('spikes_noisy_1.npy')

#### temp - in this part parameter changed and again has to run just following part
N = np.shape(spikes_noisy_1)[0]
#wup_time = 1000
#spikes = spikes[:, wup_time:]
sim_dur = np.shape(spikes_noisy_1)[1]
t_len = int(sim_dur)
...
calcium_noisy = np.concatenate((calcium_noisy_1, calcium_noisy_2), axis=1)
np.save('calcium_noisy.npy', calcium_noisy)

current_time = dtm.now()
time_str = current_time.strftime("%H:%M:%S")
with open('make-log.txt', 'a') as f:
    f.write('Finished: ' + time_str + '\n')
