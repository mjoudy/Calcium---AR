...
class BrunelNetwork:
    ...
    def save_data(self):
        ...
        name_spikes = "spikes-"+name_prefix+".zarr"
        name_adj = "connectivity-"+name_prefix+".npy"
        add_spikes = os.getcwd() + "/data/" + name_spikes
        add_adj = os.getcwd() + "/data/" +name_adj

        #with h5py.File(add_spikes, 'w') as f:
            #f.create_dataset('spikes_trains', data=self.spikes_trains)
            #f.close()
        zarr.save(add_spikes, self.spikes_trains)

        np.save(add_adj, self.adj_matrix)
